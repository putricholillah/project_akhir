            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                ...
                <table class="w-full table-fixed">
                    <thead>
                        ...
                        <th class="px-4 py-2 border">Kode Mata Kuliah</th>
                    ...
                    <tbody>
                        @if(!empty($makuls))
                            @foreach($makuls as $row)
                                <tr>
                                    <td class="px-4 py-2 border">{{ $row->kode_mk }}</td>
                                    <td class="px-4 py-2 border">{{ $row->matkul }}</td>
                                    <td class="px-4 py-2 border">{{ $row->kelas }}</td>
                                    <td class="px-4 py-2 border">{{ $row->dosen_pengampu }}</td>
                                    ...
                                    <td class="px-4 py-2 border text-center">
                                        <form action="{{ route('makuls.destroy', $row->id) }}" method="POST">
                                            <a href="{{ route('makuls.show', $row->id) }}" class="inline-flex items-center px-4 py-2 mx-2 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out bg-gray-800 border border-transparent rounded-md hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25">
                                                Show
                                            </a>
                                            <a href="{{ route('makuls.edit', $row->id) }}" class="inline-flex items-center px-4 py-2 mx-2 text-xs font-semibold tracking-widest text-white uppercase transition duration-150 ease-in-out bg-gray-800 border border-transparent rounded-md hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25">
                                                Edit
                                            </a>
                                            @csrf
                                            @method('DELETE')
